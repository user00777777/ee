...
// district:'' },
 ...
 document.getElementById('btn').addEventListener('click',()=>{
   ...
   julj.map(j=>{
   j.operator.includes(inp)?out.innerHTML= `<br> участок: <p class="p1">${j.district}</p>Номер дільниці<p class="p5">${j.namber}</p> табельный:<p  class="p4">${j.tab}</p>контролер:<p class="p2">${j.operator}</p> телефон контролера:<p class="p3">${j.tel}</p>`:console.log(inp);
   
   j.operator.includes(inp)?out.classList.add('h'):console.log('j');
   j.namber.includes(inp)?out.innerHTML=`<br> участок: <p class="p1">${j.district}</p> табельный:<p  class="p4">${j.tab}</p>контролер:<p class="p2">${j.operator}</p> телефон контролера:<p class="p3">${j.tel}</p>`:console.log(inp);
